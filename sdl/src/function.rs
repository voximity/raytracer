use crate::{
    ast::NodeKind,
    interpret::{InterpretError, Value},
};
...
pub struct Function {
    ...
    pub func: Box<dyn Send + Sync + Fn(Vec<Value>) -> Result<Value, InterpretError>>,
}
...
impl Function {
    ...
    where
        F: 'static + Send + Sync + Fn(Vec<Value>) -> Result<Value, InterpretError>,
    {
        Self {
            names,
            arg_types,
    ...
    pub fn try_eval(&self, args: Vec<Value>) -> Option<Result<Value, InterpretError>> {
